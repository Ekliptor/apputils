...
function dateFromJson(obj, datePros) {
    for (var _i = 0, datePros_1 = datePros; _i < datePros_1.length; _i++) {
        var prop = datePros_1[_i];
        if (obj[prop])
...
function dateFromUtc(year, month, date, hours, minutes, seconds, ms) {
    var utcDate = new Date(year, month, date, hours, minutes, seconds, ms);
...
function getOtherServerDate(serverTimezoneOffset, ms) {
 ...
 * @param utc
 */
function toDateTimeStr(now, withSeconds, utc) {
    if (withSeconds === void 0) { withSeconds = true; }
    if (utc === void 0) { utc = true; }
    if (utc === true) {
        var date_1 = now.getUTCFullYear() + '-' + utils.padNumber(now.getUTCMonth() + 1, 2) + '-' + utils.padNumber(now.getUTCDate(), 2);
        if (withSeconds)
            date_1 += ' ' + utils.padNumber(now.getUTCHours(), 2) + ':' + utils.padNumber(now.getUTCMinutes(), 2) + ':' + utils.padNumber(now.getUTCSeconds(), 2);
        return date_1;
    }
    var date = now.getFullYear() + '-' + utils.padNumber(now.getMonth() + 1, 2) + '-' + utils.padNumber(now.getDate(), 2);
    if (withSeconds)
...
function formatDateTime(dateFormat, date) {
    var result = formatDate(dateFormat, date);
...
function parseElapsedUnixProcessDate(elapsedStr) {
    ...
    if (!elapsedStr)
        return elapsed;
    var parts = elapsedStr.split("-"); // 2-16:04:57
    var i = 0; // the index for time
    if (parts.length === 2) { // days and time
        elapsed = utils.date.dateAdd(elapsed, 'day', -1 * parseInt(parts[0]));
        i = 1;
    }
    var timeParts = parts[i].split(":"); // valid is 12:04:31 and 04:31
...
function getElapsedUnixProcessTime(PID) {
    return new Promise(function (resolve, reject) {
        ...
        var child = exec("ps axo pid,etime | grep " + PID + " | awk '{print $2}'", options, function (err, stdout, stderr) {
            if (err)
                return reject(err);
            stdout = stdout.trim();
            if (!stdout)
...
function overlaps(range1, range2, allowEqual) {
    if (allowEqual === void 0) { allowEqual = false; }
    if (!range1 || !range2)
        return false;
    var e1start = range1.start.getTime();
    var e1end = range1.end.getTime();
    var e2start = range2.start.getTime();
    var e2end = range2.end.getTime();
    if (allowEqual === true)
 ...
 */
function toHourMinuteStr(hours, isMinutes) {
    if (isMinutes === void 0) { isMinutes = false; }
    if (isMinutes)
        hours /= 60;
    var fullHours = Math.floor(hours);
    var minutes = Math.floor((hours - fullHours) * 60);
 ...
 */
function toDayHourStr(days, isHours) {
    if (isHours === void 0) { isHours = false; }
    if (isHours)
        days /= 24;
    var fullDays = Math.floor(days);
    var hours = Math.floor((days - fullDays) * 24);
